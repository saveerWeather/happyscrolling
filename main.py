...
def init_database():
    ...
    conn.commit()
    conn.close()
    print(f"✅ Database initialized ({'PostgreSQL' if USE_POSTGRES and DATABASE_URL else 'SQLite'})")
...
def save_to_database(sender_email, core_link, received_date):
    ...
    try:
        if USE_POSTGRES and DATABASE_URL:
            cursor.execute('''
                INSERT INTO feed_items (sender_email, core_link, received_date, processed_date)
                VALUES (%s, %s, %s, %s)
